...
export function register(server: FastMCP) {
  server.addTool({
    ...
    description:
      'Merges cells in a table into a single cell. Specify the top-left cell and the span. ' +
      'Content from all merged cells is concatenated into the resulting cell. ' +
    ...
    parameters: DocumentIdParameter.extend({
      ...
      rowIndex: z
        ...
        .int()
        .min(0)
        .describe('Row index of the top-left cell of the merge range (0-based).'),
      columnIndex: z
        .number()
        .int()
        .min(0)
        .describe('Column index of the top-left cell of the merge range (0-based).'),
      rowSpan: z
        .number()
        .int()
        .min(1)
        .describe('Number of rows to merge.'),
      columnSpan: z
        .number()
        .int()
        .min(1)
        .describe('Number of columns to merge.'),
      tabId: z
        .string()
        .optional()
        .describe(
    ...
    execute: async (args, { log }) => {
      ...
      log.info(
        `Merging table cells in doc ${args.documentId} at table index ${args.tableStartIndex}: ` +
      ...
      try {
        ...
        return (
          `Successfully merged cells [${args.rowIndex},${args.columnIndex}] ` +
          `span ${args.rowSpan}x${args.columnSpan} in table at index ${args.tableStartIndex}.`
        );
      } catch (error: any) {
        log.error(`Error merging table cells: ${error.message || error}`);
